...
class ConversationEngine {
    ...
    messageGenerator;
    botManager;
    constructor(contextTracker, messageGenerator, botManager) {
        this.contextTracker = contextTracker;
        this.messageGenerator = messageGenerator;
        this.botManager = botManager;
    }
    async generateMessage(bot) {
        ...
        if (roll < 30)
            return this.messageGenerator.generateReaction(bot.getId());
        if (roll < 55)
            return this.messageGenerator.generateStreamerQuestion(bot.getId());
        if (roll < 75)
            return this.messageGenerator.generateChatComment(bot.getId());
        if (roll < 90)
            return this.messageGenerator.generateIdle(bot.getId());
